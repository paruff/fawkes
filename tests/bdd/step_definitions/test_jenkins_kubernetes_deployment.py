...
def check_namespace_exists(jenkins_context, kubectl_helper):
    ...
    assert jenkins_context['namespace'] in namespaces, \
        f"Namespace {jenkins_context['namespace']} not found"
...
def check_jenkins_deployment(jenkins_context, kubectl_helper):
    ...
    jenkins_context['deployment'] = kubectl_helper.get_deployment(
        'jenkins',
        jenkins_context['namespace']
    )
...
def verify_jenkins_running(jenkins_context):
    ...
    assert jenkins_context['deployment'] is not None, \
        "Jenkins deployment not found"
...
def verify_jenkins_pod_ready(jenkins_context, kubectl_helper):
    ...
    pods = kubectl_helper.get_pods_by_label(
...
def verify_jenkins_service(jenkins_context, kubectl_helper):
    ...
    service = kubectl_helper.get_service(
        jenkins_context['jenkins_service'],
        jenkins_context['namespace']
    )
...
def verify_k8s_cloud_namespace(jenkins_context, namespace):
    ...
    assert k8s_cloud.get('namespace') == namespace, \
        f"Expected namespace {namespace}, got {k8s_cloud.get('namespace')}"
...
def verify_jenkins_url(jenkins_context, url):
    ...
    k8s_cloud = jenkins_context.get('k8s_cloud', {})
    assert k8s_cloud.get('jenkinsUrl') == url, \
        f"Expected URL {url}, got {k8s_cloud.get('jenkinsUrl')}"
...
def verify_jenkins_tunnel(jenkins_context, tunnel):
    ...
    k8s_cloud = jenkins_context.get('k8s_cloud', {})
    assert k8s_cloud.get('jenkinsTunnel') == tunnel, \
        f"Expected tunnel {tunnel}, got {k8s_cloud.get('jenkinsTunnel')}"
...
def login_with_admin_creds(jenkins_context, jenkins_api_helper):
    ...
    success = jenkins_api_helper.login(
        jenkins_context['admin_username'],
        jenkins_context['admin_password']
    )
...
def verify_instance_capacity(jenkins_context):
    ...
    templates = jenkins_context.get('agent_templates', [])
    for template in templates:
        assert 'instanceCapStr' in template, \
            f"Template {template.get('name')} missing instance capacity"
...
def verify_system_message(jenkins_context, message):
    ...
    assert message in actual_message, \
        f"Expected '{message}' in system message, got '{actual_message}'"
...
def verify_executor_count(jenkins_context, count, jenkins_api_helper):
    ...
    assert executor_count == count, \
        f"Expected {count} executors, got {executor_count}"
