...
def step_meeting_has_agenda(context):
    ...
    assert 'Agenda' in context.meeting_content, \
        "Meeting template should include agenda"
...
def step_meeting_has_time(context):
    ...
    assert 'minutes' in context.meeting_content.lower(), \
        "Meeting template should include time allocations"
...
def step_meeting_has_actions(context):
    ...
    assert 'Action Items' in context.meeting_content, \
        "Meeting template should include action items section"
...
def step_feedback_has_relevance(context):
    ...
    assert 'Relevance' in context.feedback_content, \
        "Feedback form should include relevance assessment"
...
def step_feedback_has_priority(context):
    ...
    assert 'Priority' in context.feedback_content, \
        "Feedback form should include priority assessment"
...
def step_feedback_has_approach(context):
    ...
    assert 'Approach' in context.feedback_content, \
        "Feedback form should include approach evaluation"
...
def step_feedback_has_adoption(context):
    ...
    assert 'Adoption' in context.feedback_content, \
        "Feedback form should include adoption planning"
...
def step_directory_has_template(context):
    ...
    assert 'Member Name' in context.member_directory_content or 'template' in context.member_directory_content.lower(), \
...
def step_target_size(context):
    ...
    assert '5-7' in context.charter_content, \
        "Charter should specify 5-7 members as target size"
...
def step_criteria_time(context):
    ...
    assert 'hours per quarter' in context.charter_content.lower() or 'time commitment' in context.charter_content.lower(), \
...
def step_process_self_nomination(context):
    ...
    assert 'self-nomination' in context.charter_content.lower() or 'nominate themselves' in context.charter_content.lower(), \
...
def step_process_team_nomination(context):
    ...
    assert 'team nomination' in context.charter_content.lower() or 'maintainers can nominate' in context.charter_content.lower(), \
...
def step_process_community_nomination(context):
    ...
    assert 'community nomination' in context.charter_content.lower() or 'community members can nominate' in context.charter_content.lower(), \
...
def step_process_onboarding(context):
    ...
    assert 'onboarding' in context.charter_content.lower(), \
        "Process should include onboarding procedures"
...
def step_quarterly_meetings(context):
    ...
    assert 'quarterly' in context.charter_content.lower(), \
        "Quarterly strategic meetings should be defined"
...
def step_agenda_roadmap(context):
    ...
    assert 'roadmap' in context.charter_content.lower(), \
        "Agenda should include roadmap review"
...
def step_agenda_deep_dive(context):
    ...
    assert 'deep dive' in context.charter_content.lower(), \
        "Agenda should include deep dive topic"
...
def step_input_early_access(context):
    ...
    assert 'early access' in context.charter_content.lower(), \
        "Input mechanisms should include early access testing"


@then('input mechanisms should include RFC reviews')
def step_input_rfc(context):
    """Check RFC reviews as input mechanism"""
    assert 'rfc' in context.charter_content.lower(), \
        "Input mechanisms should include RFC reviews"
...
def step_channel_name(context):
    """Check channel name"""
    assert 'cab-advisory-board' in context.charter_content.lower(), \
        'Channel name should be "cab-advisory-board"'


@then('the channel should be private')
def step_channel_private(context):
    """Check channel is private"""
    assert 'private' in context.charter_content.lower(), \
...
def step_email_option(context):
    """Check email option"""
    assert 'email' in context.charter_content.lower(), \
        "Email communication option should be available"
...
def step_channel_purpose(context):
    ...
    assert 'purpose' in context.charter_content.lower(), \
        "Channel purpose should be documented"
...
def step_channel_members_cab(context):
    ...
    assert 'advisory-board' in context.charter_content.lower(), \
        "Channel members should include advisory board team"
...
def step_recognition_mechanisms(context):
    ...
    assert 'recognition' in context.charter_content.lower(), \
        "Public recognition mechanisms should be defined"
...
def step_recognition_blog(context):
    ...
    assert 'blog' in context.charter_content.lower(), \
        "Blog post opportunities should be mentioned"
...
def step_recognition_badges(context):
    ...
    assert 'badge' in context.charter_content.lower(), \
        "Digital badges should be offered"


@then('swag options should be mentioned')
def step_recognition_swag(context):
    """Check swag options"""
    assert 'swag' in context.charter_content.lower(), \
        "Swag options should be mentioned"
...
def step_metrics_impact(context):
    ...
    assert 'impact' in context.charter_content.lower(), \
        "Impact metrics should be defined"
...
def step_metrics_reporting(context):
    ...
    assert 'quarterly' in context.charter_content.lower(), \
        "Reporting cadence should be quarterly"
...
def step_onboarding_week1(context):
    ...
    assert 'week 1' in context.welcome_packet_content.lower() or 'first week' in context.welcome_packet_content.lower(), \
...
def step_onboarding_mattermost(context):
    ...
    assert 'mattermost' in context.welcome_packet_content.lower(), \
        "Mattermost access should be included"


@then('GitHub team access should be included')
def step_onboarding_github(context):
    """Check GitHub team access"""
    assert 'github' in context.welcome_packet_content.lower(), \
        "GitHub team access should be included"


@then('onboarding call should be scheduled')
def step_onboarding_call(context):
    """Check onboarding call"""
    assert 'onboarding call' in context.welcome_packet_content.lower() or 'intro call' in context.welcome_packet_content.lower(), \
...
def step_onboarding_roadmap(context):
    ...
    assert 'roadmap' in context.welcome_packet_content.lower(), \
        "Roadmap review should be assigned"
...
def step_nomination_exists(context):
    ...
    assert file_exists('docs/research/templates/cab-nomination.md'), \
        "Nomination template should exist"


@then('the meeting agenda template should exist')
def step_meeting_agenda_exists(context):
    """Check meeting agenda template exists"""
    assert file_exists('docs/research/templates/cab-meeting-agenda.md'), \
        "Meeting agenda template should exist"


@then('the feedback form template should exist')
def step_feedback_form_exists(context):
    """Check feedback form template exists"""
    assert file_exists('docs/research/templates/cab-feedback-form.md'), \
        "Feedback form template should exist"


@then('the welcome packet should exist')
def step_welcome_packet_exists(context):
    """Check welcome packet exists"""
    assert file_exists('docs/research/data/cab-welcome-packet.md'), \
        "Welcome packet should exist"
...
def step_templates_proper_locations(context):
    ...
    assert context.all_templates_exist, \
        "All templates should be in proper locations"
...
def step_templates_formatted(context):
    ...
    for filepath in [
        ...
        assert content and len(content) > 100, \
            f"Template {filepath} should be properly formatted with content"
...
def step_charter_discoverable(context):
    ...
    assert file_exists('docs/CUSTOMER_ADVISORY_BOARD.md'), \
        "CAB charter should be discoverable"
...
def step_howto_in_section(context):
    ...
    assert file_exists('docs/how-to/run-advisory-board-meetings.md'), \
        "How-to guide should be in how-to section"
...
def step_templates_in_section(context):
    ...
    assert file_exists('docs/research/templates/cab-nomination.md'), \
        "Templates should be in research templates section"
...
def step_directory_discoverable(context):
    ...
    assert file_exists('docs/CUSTOMER_ADVISORY_BOARD_MEMBERS.md'), \
        "Member directory should be discoverable"
...
def step_has_doc_info(context):
    ...
    assert any('document information' in section.lower() for section in context.charter_sections) or \
           'Version' in context.charter_content, \
...
def step_has_meeting_cadence(context):
    """Check meeting cadence section"""
    assert any('meeting cadence' in section.lower() or 'cadence' in section.lower() for section in context.charter_sections), \
        "Charter should have meeting cadence section"
...
def step_has_feedback_process(context):
    """Check feedback process section"""
    assert any('feedback process' in section.lower() or 'feedback' in section.lower() for section in context.charter_sections), \
        "Charter should have feedback process section"
...
def step_has_confidentiality(context):
    ...
    assert any('confidentiality' in section.lower() or 'ip' in section.lower() for section in context.charter_sections) or \
           'Confidentiality' in context.charter_content, \
...
def step_has_success_metrics(context):
    """Check success metrics section"""
    assert any('success metrics' in section.lower() or 'metrics' in section.lower() for section in context.charter_sections), \
...
def step_has_faqs(context):
    """Check FAQs section"""
    assert any('faq' in section.lower() for section in context.charter_sections), \
        "Charter should have FAQs section"


@then('it should have appendix with related documents')
def step_has_appendix(context):
    """Check appendix section"""
    assert any('appendix' in section.lower() for section in context.charter_sections) or \
           'Related Documents' in context.charter_content, \
...
def step_pre_meeting_documented(context):
    ...
    assert 'pre-meeting' in context.howto_guide_content.lower(), \
        "Pre-meeting process should be documented"
...
def step_during_meeting_documented(context):
    ...
    assert 'during meeting' in context.howto_guide_content.lower() or 'facilitation' in context.howto_guide_content.lower(), \
...
def step_post_meeting_documented(context):
    ...
    assert 'post-meeting' in context.howto_guide_content.lower(), \
        "Post-meeting follow-up should be documented"
...
def step_action_github_issues(context):
    ...
    assert 'github issue' in context.howto_guide_content.lower(), \
        "GitHub issues should be created for action items"


@then('issues should be labeled with "cab-feedback"')
def step_action_labels(context):
    """Check issue labels"""
    assert 'cab-feedback' in context.howto_guide_content.lower(), \
        'Issues should be labeled with "cab-feedback"'


@then('issues should reference meeting date')
def step_action_meeting_date(context):
    """Check meeting date reference"""
    assert 'meeting date' in context.howto_guide_content.lower() or 'reference' in context.howto_guide_content.lower(), \
...
def step_status_forming(context):
    ...
    assert 'forming' in context.member_directory_content.lower(), \
        'Status should indicate "Forming"'


@then('the current size should be shown')
def step_current_size_shown(context):
    """Check current size"""
    assert 'current size' in context.member_directory_content.lower() or 'accepting' in context.member_directory_content.lower(), \
...
def step_nomination_instructions(context):
    ...
    assert 'how to join' in context.member_directory_content.lower() or 'apply' in context.member_directory_content.lower(), \
...
def step_contact_info(context):
    ...
    assert 'contact' in context.member_directory_content.lower() or 'email' in context.member_directory_content.lower(), \
