...
def step_golden_path_configured(context):
    ...
    try:
        ...
        assert 'fawkes-pipeline-library' in config_yaml, \
            "Golden Path library not configured"
...
def step_trivy_available_in_pod(context):
    ...
    assert context.trivy_container_available, \
        "Trivy container not available in pipeline pod"
...
def step_pipeline_fails(context):
    ...
    assert context.pipeline_status == 'FAILURE', \
        f"Expected FAILURE but got {context.pipeline_status}"


@then('the build status should be {status}')
def step_build_status(context, status):
    """Verify build status matches expected."""
    assert context.pipeline_status == status, \
        f"Expected {status} but got {context.pipeline_status}"
...
def step_pod_in_state(context, state):
    ...
    assert pod.status.phase == state, \
        f"Pod in {pod.status.phase} state, expected {state}"
