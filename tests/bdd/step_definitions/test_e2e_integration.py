...
def step_components_healthy(context):
    ...
    for row in context.table:
        ...
        pods = v1.list_namespaced_pod(namespace=namespace, label_selector=f"app.kubernetes.io/name={component}")

        running_pods = [p for p in pods.items if p.status.phase == "Running"]

        if not running_pods:
            # Try alternative label
            pods = v1.list_namespaced_pod(namespace=namespace, label_selector=f"app={component}")
            running_pods = [p for p in pods.items if p.status.phase == "Running"]
...
def step_templates_available(context):
    ...
    template_dirs = ["templates/python-service", "templates/java-service", "templates/nodejs-service"]
...
def step_no_manual_intervention(context):
    ...
    try:
        apps = custom_api.list_namespaced_custom_object(
            group="argoproj.io", version="v1alpha1", namespace="fawkes", plural="applications"
        )
...
def step_use_golden_path_template(context):
    ...
    required_skeleton_files = ["skeleton/Jenkinsfile", "skeleton/Dockerfile", "skeleton/catalog-info.yaml"]
