...
scenarios("../features/feedback-widget.feature")
scenarios("../features/multi-channel-feedback.feature")
...
@given(parsers.parse('the feedback service is deployed in namespace "{namespace}"'))
...
@given(parsers.parse("there is existing feedback with ID {feedback_id}"))
...
@when(parsers.parse('I check for the feedback-service deployment in namespace "{namespace}"'))
...
@when(parsers.parse('I check for the CloudNativePG cluster in namespace "{namespace}"'))
...
@when(parsers.parse('I submit feedback with rating {rating:d} and category "{category}"'))
...
@when(parsers.parse("I try to submit feedback with invalid rating {rating:d}"))
...
@when(parsers.parse('I update feedback status to "{status}"'))
...
@when(parsers.parse('I check the ingress configuration in namespace "{namespace}"'))
...
@then(parsers.parse('the deployment "{deployment_name}" should exist'))
...
@then(parsers.parse("the deployment should have at least {count:d} ready replica"))
@then(parsers.parse("the deployment should have at least {count:d} ready replicas"))
...
@then(parsers.parse('the cluster "{cluster_name}" should exist'))
...
@then(parsers.parse("the request should be rejected with status {status:d}"))
...
@then(parsers.parse('the ingress should use ingressClassName "{class_name}"'))
...
@then(parsers.parse('the proxy should include endpoint "{endpoint}"'))
...
@then(parsers.parse('the proxy target should be "{target}"'))
...
@given(parsers.parse('Grafana is deployed in namespace "{namespace}"'))
...
@when(parsers.parse('I check for the feedback-bot deployment in namespace "{namespace}"'))
...
@when(parsers.parse('I check for the feedback-automation CronJob in namespace "{namespace}"'))
...
@then(parsers.parse('the CLI should have a "{command}" command'))
...
@then(parsers.parse('the CronJob "{cronjob_name}" should exist'))
...
@then(parsers.parse('the dashboard file "{filename}" should exist'))
...
def step_service_exposes_metrics(context):
    ...
    has_monitor = any(
        "feedback-service" in sm.get("metadata", {}).get("name", "") for sm in context["service_monitors"]
    )
...
def step_servicemonitor_feedback_bot(context):
    ...
    has_monitor = any(
        "feedback-bot" in sm.get("metadata", {}).get("name", "") for sm in context["observability_monitors"]
    )
