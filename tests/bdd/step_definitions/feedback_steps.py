...
def step_service_has_automation_endpoint(context):
    ...
    with open(service_code_path, 'r') as f:
        content = f.read()
        assert "automation" in content.lower() or "process_validated" in content, \
            "Service missing automation endpoint"
...
def step_service_has_limits(context):
    ...
    assert context.security_checks.get("feedback_service", False), \
        "feedback-service missing resource limits"


@then('the feedback-bot should have CPU and memory limits')
def step_bot_has_limits(context):
    """Verify bot has resource limits."""
    assert context.security_checks.get("feedback_bot", False), \
        "feedback-bot missing resource limits"
...
def step_all_components_non_root(context):
    ...
    for deployment_name in ["feedback-service", "feedback-bot"]:
        try:
            ...
            assert security_context is not None or deployment.spec.template.spec.containers[0].security_context is not None
        except:
...
def step_all_components_security_context(context):
    ...
    manifest_files = [
        "platform/apps/feedback-service/deployment.yaml",
        "platform/apps/feedback-bot/deployment.yaml"
    ]
...
def step_secrets_managed(context):
    ...
    secret_files = [
        "platform/apps/feedback-service/secrets.yaml",
        "platform/apps/feedback-bot/secret.yaml"
    ]
