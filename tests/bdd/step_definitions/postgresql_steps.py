...
def cloudnativepg_installed():
    ...
    result = subprocess.run(
        ['kubectl', 'get', 'deployment', '-n', 'cloudnativepg-system',
...
def verify_ha_cluster():
    ...
    result = subprocess.run(
        ['kubectl', 'get', 'cluster', 'db-focalboard-dev', '-n', 'fawkes',
...
def verify_persistent_storage():
    ...
    result = subprocess.run(
        ['kubectl', 'get', 'pvc', '-n', 'fawkes',
         '-l', 'cnpg.io/cluster=db-focalboard-dev',
         '-o', 'jsonpath={.items[*].metadata.name}'],
        capture_output=True,
        text=True
    )
...
def verify_credentials_secret():
    ...
    result = subprocess.run(
        ['kubectl', 'get', 'secret', 'db-focalboard-credentials',
         '-n', 'fawkes', '-o', 'jsonpath={.metadata.name}'],
        capture_output=True,
        text=True
    )
    # In CI/CD without a real cluster, we skip the actual check
