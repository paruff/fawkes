...
def review_all_personas(context=None):
    ...
    for persona_file in persona_files:
        if persona_file.exists():
            with open(persona_file, "r", encoding="utf-8") as f:
                context["all_personas"].append({"file": persona_file.name, "content": f.read()})
...
def check_goals_and_metrics(context):
    ...
    for persona in context.get("all_personas", []):
        ...
        results.append(
            {"file": persona["file"], "has_goals": has_goals, "goal_count": goal_count, "has_metrics": has_metrics}
        )
...
def validation_has_evidence(context):
    ...
    content = context.get("validation_content", "")
    assert "Evidence" in content or "Validation" in content, "Validation document missing supporting evidence"
...
def validation_has_privacy(context):
    ...
    content = context.get("validation_content", "")
    assert "Privacy" in content or "Ethics" in content, "Validation document missing privacy/ethics information"
...
def personas_linked_from_research(context):
    ...
    if research_readme.exists():
        ...
        assert "persona" in content.lower() or "user" in content.lower(), "Research README should reference personas"
...
def readme_has_update_procedures(context):
    ...
    assert "Update" in content or "Review" in content, "README missing update procedures"
...
def has_continuous_validation_process(context):
    ...
    content = context["lifecycle_docs"].get("validation", "")
    assert "continuous" in content.lower() or "ongoing" in content.lower(), "Missing continuous validation process"
