...
class SecretScanner:
    ...
    PLACEHOLDER_PATTERNS = [
        'CHANGEME', 'PLACEHOLDER', 'REPLACE_WITH', 'changeme',
        'your-', 'example-', 'CHANGE_ME', 'REPLACE_ME',
        'not-a-secure', 'dev-', 'test-', 'changeme-'
    ]
    ...
    def has_real_values(self, string_data: Dict, data: Dict) -> bool:
        ...
        for key, value in {**string_data, **data}.items():
...
class SealedSecretGenerator:
    ...
    def __init__(self, controller_name: str = 'sealed-secrets', 
    ...
    def seal_secret(self, secret: Dict) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"Error sealing {secret['name']}: {result.stderr}", file=sys.stderr)
                return None
            
            return result.stdout
            
        except subprocess.TimeoutExpired:
            print(f"Timeout sealing {secret['name']}", file=sys.stderr)
            return None
...
Examples:
  ...
  python3 scripts/seal-secrets.py --namespace fawkes
    ...
    parser.add_argument('--dry-run', action='store_true',
                        help='Show what would be sealed without writing files')
    parser.add_argument('--cert', type=str,
                        help='Path to certificate file for offline sealing')
    parser.add_argument('--controller-name', type=str, default='sealed-secrets',
                        help='Sealed Secrets controller name (default: sealed-secrets)')
    parser.add_argument('--controller-namespace', type=str, default='kube-system',
                        help='Sealed Secrets controller namespace (default: kube-system)')
    parser.add_argument('--scope', type=str, default='strict',
                        choices=['strict', 'namespace-wide', 'cluster-wide'],
                        help='Sealing scope (default: strict)')
    parser.add_argument('--namespace', type=str,
                        help='Only seal secrets in this namespace')
    parser.add_argument('--force', action='store_true',
                        help='Overwrite existing sealed secrets')
    parser.add_argument('--include-placeholders', action='store_true',
                        help='Also seal secrets with only placeholder values')
    ...
    for secret in secrets:
        ...
        if output_path.exists() and not args.force:
            print(f"Skipping {secret['name']}: {output_path} already exists (use --force to overwrite)")
            continue
