        ...
        | Metric | Command |
        |---|---|
        | Deployment Frequency | `git log --since='30 days ago' --merges --first-parent --pretty=format:%ad --date=short | uniq -c` |
        | Lead Time for Changes | `gh pr list --search 'merged:>=2026-01-01' --json number,createdAt,mergedAt` |
        | Change Failure Rate | `gh pr list --search 'label:incident-fix merged:>=2026-01-01' --json number` |
        | MTTR | `gh issue list --label incident --state closed --json number,createdAt,closedAt` |
        | Rework Rate | `git log --since='30 days ago' --merges --first-parent --pretty=%s | grep -E '^(fix|hotfix|revert)(\(|:)'` |
        | AI Readiness Coverage | `./scripts/check-ai-readiness.sh` |
        | AI Credit Burn Rate | `bash scripts/token-audit.sh` and compare with org billing dashboard export |
...
", (rework / total) * 100 }'
        ...
        2. Run `bash scripts/token-audit.sh --save` to append the current always-on context cost snapshot.
