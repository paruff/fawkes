...
import com.unisys.verfuture.utilities.Helper;
 ...
 *
 */
public class UpdateI9FormPage extends TestBase {
	Helper helper = new Helper();
	@FindBy(css = "input[name='fullLegalName']")
	...
	public UpdateI9FormPage(WebDriver driver) {
	...
	public void fillformWithDetails() throws InterruptedException, AWTException {
		String ranVal = helper.getRandomNumber_Str(100);
		TestUtil.waitForVisibilityofElement(fulllegalname_Txt);
		String updatedlegalName = fulllegalname_Txt.getText();
		fulllegalname_Txt.sendKeys(updatedlegalName + "_" + ranVal);
		TestUtil.waitForVisibilityofElement(alias_Txt);
		String updatedaliasName = alias_Txt.getText();
		alias_Txt.sendKeys(updatedaliasName + "_" + ranVal);
		TestUtil.waitForVisibilityofElement(dob_Txt);
		String updateddob = dob_Txt.getText();
		dob_Txt.sendKeys(updateddob + "_" + ranVal);
		TestUtil.waitForVisibilityofElement(addr_Txt);
		String updatedaddress = addr_Txt.getText();
		addr_Txt.sendKeys(updatedaddress + "_" + ranVal);
		// TestUtil.waitForVisibilityofElement(status);
		Select statusDrpdn = new Select(statusDrp);
		((Select) statusDrpdn).selectByVisibleText("Green Card Holder");
	}
