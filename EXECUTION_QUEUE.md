...
| #2117 | Wire real ArgoCD/Tekton/Alertmanager metrics into Prometheus | **New P0.** `dora.yml`'s recording rules (now deployed via `prometheus-application.yaml`) compute from metric names that don't exist on `mac-mini-k3s` — verified live: zero `argocd_*`/`tekton_*`/`alertmanager_alert_*` series exist; ArgoCD has no metrics Service here; Tekton isn't deployed on this cluster at all. This is the actual remaining work behind the native-PromQL decision. | **OPEN** — not started |
...
| #2032 | java-fawkes-path golden path (Phase 2/Beta) | PR #2114 added a design spec only; implementation lives in the separate `paruff/java-fawkes-path` repo and is estimated ~16h — too large for this queue's per-item granularity. Phased plan posted as a comment on #2032. |
