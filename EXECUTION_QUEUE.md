...
| #2117 | Wire real ArgoCD/Tekton/Alertmanager metrics into Prometheus | **DONE for the ArgoCD + Tekton portion, on `fawkes-dev-aks`** (2026-09-16, PR #2129, not yet merged). Real metric names confirmed live (`argocd_app_sync_total{phase}`, `tekton_pipelines_controller_pipelinerun_duration_seconds_bucket`), recording rules rewritten to match, all 5 DORA metrics verified end-to-end including through Grafana's actual query API (found and fixed a separate Grafana 13.2.1 regression along the way, #2130). Alertmanager-sourced metrics (real MTTR) still not wired — FDRT/MTTR and Rework Rate ship as honestly-labeled proxies, not textbook definitions. | **MOSTLY DONE** — ArgoCD/Tekton portion verified live; Alertmanager wiring remains |
