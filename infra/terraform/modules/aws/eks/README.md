...
module "eks" {
  ...
  endpoint_public_access  = false  # Secure default: private-only cluster endpoint
  api_server_authorized_ip_ranges = ["203.0.113.10/32"]  # Used only if endpoint_public_access is true
...
3. **Public Access (secure default)**: `endpoint_public_access` defaults to **`false`** — clusters are private-only by default
4. **RBAC**: Enable Kubernetes RBAC (enabled by default)
5. **IRSA**: Use IAM Roles for Service Accounts for pod-level permissions
6. **Node Security**: Deploy nodes in private subnets
7. **Audit Logging**: Enable audit logs for compliance

## Migration Note: `endpoint_public_access` default change

The `endpoint_public_access` default changed from `true` to `false` (Trivy:
"EKS Clusters should have the public access disabled"). This closes a public
API-server endpoint by default; the private endpoint (`endpoint_private_access`,
default `true`) remains the reachable path for in-VPC clients.

- **Existing consumers** relying on the old default must set
  `endpoint_public_access = true` **explicitly** and pair it with a scoped
  `api_server_authorized_ip_ranges` (never `0.0.0.0/0`).
- **New consumers** get the private-only posture with no configuration.
- The module currently has no live callers, so no in-repo deployment is affected.
