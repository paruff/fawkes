...
def test_pulse_survey_response_invalid_values():
    ...
    with pytest.raises(ValidationError):
        PulseSurveyResponse(flow_state_days=8.0, valuable_work_pct=70.0, cognitive_load=3.0, friction_incidents=False)

    # Test invalid valuable_work_pct (> 100)
    with pytest.raises(ValidationError):
        PulseSurveyResponse(flow_state_days=3.0, valuable_work_pct=150.0, cognitive_load=3.0, friction_incidents=False)

    # Test invalid cognitive_load (> 5)
    with pytest.raises(ValidationError):
        PulseSurveyResponse(flow_state_days=3.0, valuable_work_pct=70.0, cognitive_load=6.0, friction_incidents=False)
...
def test_survey_distribution_request_validation():
    ...
    # Valid request
    request = SurveyDistributionRequest(type="pulse", test_mode=True, test_users=["user1@example.com"])
