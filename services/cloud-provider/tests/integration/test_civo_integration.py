...
pytestmark = pytest.mark.skipif(
    not os.getenv("CIVO_TOKEN"), reason="CIVO_TOKEN environment variable not set. Set it to run integration tests."
)
...
class TestCivoIntegrationStorage:
    ...
    def test_storage_lifecycle(self, civo_provider, unique_name):
        ...
        try:
            # Create storage
            config = StorageConfig(name=storage_name, region=civo_provider.region, metadata={"max_size_gb": 100})
