...
discovery_interviews_total = Gauge(
    'discovery_interviews_total',
    'Total number of interviews conducted'
)

discovery_interviews_completed = Gauge(
    'discovery_interviews_completed',
    'Number of completed interviews'
)

discovery_interviews_by_status = Gauge(
    'discovery_interviews_by_status',
    'Interviews grouped by status',
    ['status']
)

discovery_insights_total = Gauge(
    'discovery_insights_total',
    'Total number of discovery insights captured'
)

discovery_insights_validated = Gauge(
    'discovery_insights_validated',
    'Number of validated discovery insights'
)

discovery_insights_by_status = Gauge(
    'discovery_insights_by_status',
    'Discovery insights grouped by status',
    ['status']
)

discovery_insights_by_category = Gauge(
    'discovery_insights_by_category',
    'Discovery insights grouped by category',
    ['category']
)

discovery_insights_by_source = Gauge(
    'discovery_insights_by_source',
    'Discovery insights grouped by source',
    ['source']
)

discovery_experiments_total = Gauge(
    'discovery_experiments_total',
    'Total number of experiments run'
)

discovery_experiments_completed = Gauge(
    'discovery_experiments_completed',
    'Number of completed experiments'
)

discovery_experiments_by_status = Gauge(
    'discovery_experiments_by_status',
    'Experiments grouped by status',
    ['status']
)

discovery_experiments_validated = Gauge(
    'discovery_experiments_validated',
    'Number of validated experiments'
)

discovery_experiments_avg_roi = Gauge(
    'discovery_experiments_avg_roi_percentage',
    'Average ROI percentage of experiments'
)

discovery_features_total = Gauge(
    'discovery_features_total',
    'Total number of features tracked'
)

discovery_features_validated = Gauge(
    'discovery_features_validated',
    'Number of validated features'
)

discovery_features_shipped = Gauge(
    'discovery_features_shipped',
    'Number of shipped features'
)

discovery_features_by_status = Gauge(
    'discovery_features_by_status',
...
discovery_avg_time_to_validation_days = Gauge(
    ...
    'Average time from insight capture to validation (days)'
)

discovery_avg_time_to_ship_days = Gauge(
    'discovery_avg_time_to_ship_days',
    'Average time from feature proposal to ship (days)'
)

discovery_validation_rate = Gauge(
    'discovery_validation_rate',
    'Percentage of insights that get validated'
)

discovery_feature_adoption_rate = Gauge(
    'discovery_feature_adoption_rate',
    'Average feature adoption rate percentage'
)

discovery_interviews_last_7d = Gauge(
    'discovery_interviews_last_7d',
    'Number of interviews conducted in last 7 days'
)

discovery_interviews_last_30d = Gauge(
    'discovery_interviews_last_30d',
    'Number of interviews conducted in last 30 days'
)

discovery_insights_last_7d = Gauge(
    'discovery_insights_last_7d',
    'Number of insights captured in last 7 days'
)

discovery_insights_last_30d = Gauge(
    'discovery_insights_last_30d',
    'Number of insights captured in last 30 days'
)

discovery_team_performance = Gauge(
    'discovery_team_performance',
...
def update_prometheus_metrics(db: Session):
    ...
    interviews_7d = db.query(Interview).filter(
        Interview.completed_date >= seven_days_ago
    ).count()
    discovery_interviews_last_7d.set(interviews_7d)
    
    interviews_30d = db.query(Interview).filter(
        Interview.completed_date >= thirty_days_ago
    ).count()
    ...
    categories = db.query(
        DiscoveryInsight.category,
        func.count(DiscoveryInsight.id)
    ).group_by(DiscoveryInsight.category).all()
    ...
    # Insights by source
    sources = db.query(
        DiscoveryInsight.source,
        func.count(DiscoveryInsight.id)
    ).group_by(DiscoveryInsight.source).all()
    
    for source, count in sources:
        discovery_insights_by_source.labels(source=source).set(count)
    
    # Recent insights
    insights_7d = db.query(DiscoveryInsight).filter(
        DiscoveryInsight.captured_date >= seven_days_ago
    ).count()
    discovery_insights_last_7d.set(insights_7d)
    
    insights_30d = db.query(DiscoveryInsight).filter(
        DiscoveryInsight.captured_date >= thirty_days_ago
    ).count()
    ...
    avg_validation_time = db.query(
        ...
        DiscoveryInsight.time_to_validation_days.isnot(None)
    ).scalar()
    ...
    validated_experiments = db.query(Experiment).filter(
        Experiment.validated == True
    ).count()
    discovery_experiments_validated.set(validated_experiments)
    
    # Average ROI
    avg_roi = db.query(
        func.avg(Experiment.roi_percentage)
    ).filter(
        Experiment.roi_percentage.isnot(None)
    ).scalar()
    ...
    avg_ship_time = db.query(
        ...
        FeatureValidation.time_to_ship_days.isnot(None)
    ).scalar()
    ...
    avg_adoption = db.query(
        func.avg(FeatureValidation.adoption_rate)
    ).filter(
        FeatureValidation.adoption_rate.isnot(None)
    ).scalar()
    ...
    for team in teams:
        discovery_team_performance.labels(
            team=team.team_name, metric='interviews'
        ).set(team.interviews_conducted)
        discovery_team_performance.labels(
            team=team.team_name, metric='insights'
        ).set(team.insights_generated)
        discovery_team_performance.labels(
            team=team.team_name, metric='experiments'
        ).set(team.experiments_run)
        discovery_team_performance.labels(
            team=team.team_name, metric='features_validated'
        ).set(team.features_validated)
        discovery_team_performance.labels(
            team=team.team_name, metric='features_shipped'
        ).set(team.features_shipped)
