...
insights_time_to_action = Gauge(
    "research_insights_time_to_action_seconds", "Time from creation to publication", ["category"]
)
...
def update_prometheus_metrics(db: Session):
    ...
    for category in categories:
        ...
        if total_in_category > 0:
            published_in_category = (
                db.query(Insight).filter(Insight.category_id == category.id, Insight.status == "published").count()
            )
    ...
    categories_with_published = (
        db.query(Category)
        .join(Insight, Category.id == Insight.category_id)
        .filter(Insight.status == "published", Insight.published_at.isnot(None))
        .distinct()
        .all()
    )
    ...
    for category in categories_with_published:
        ...
        published_insights = (
            db.query(Insight)
            .filter(Insight.category_id == category.id, Insight.status == "published", Insight.published_at.isnot(None))
            .all()
        )
    ...
    last_7d_count = (
        db.query(Insight).filter(Insight.status == "published", Insight.published_at >= seven_days_ago).count()
    )
    ...
    last_30d_count = (
        db.query(Insight).filter(Insight.status == "published", Insight.published_at >= thirty_days_ago).count()
    )
