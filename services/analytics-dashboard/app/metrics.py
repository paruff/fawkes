...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.active_users = Gauge(
            ...
            'Number of active users in time range'
        )
        
        self.page_views = Gauge(
        ...
        self.feature_unique_users = Gauge(
            ...
            'Number of unique users per feature',
            ['feature_name']
        )
        ...
        self.significant_results = Counter(
            ...
            'Number of statistically significant experiment results'
        )
        ...
        self.segment_engagement = Gauge(
            ...
            'Average engagement score per segment',
            ['segment_name']
        )
        ...
        self.funnel_step_completion = Gauge(
            ...
            'Completion rate per funnel step',
            ['funnel_name', 'step']
        )
        ...
        self.data_refresh_duration = Histogram(
            'analytics_data_refresh_duration_seconds',
            'Time taken to refresh analytics data'
        )
    ...
    def update_feature_metrics(self, features: list):
        ...
        for feature in features:
            ...
            self.feature_adoption_rate.labels(feature_name=feature_name).set(
                feature.get('adoption_rate', 0)
            )
            self.feature_unique_users.labels(feature_name=feature_name).set(
                feature.get('unique_users', 0)
            )
        ...
        for exp in experiments:
            ...
            for variant in exp.get('variants', []):
                ...
                self.experiment_conversions.labels(
                    ...
                    variant=variant_id
                ).set(variant.get('conversions', 0))
                self.experiment_conversion_rate.labels(
                    experiment_id=exp_id,
                    variant=variant_id
                ).set(variant.get('conversion_rate', 0))
    ...
    def update_segment_metrics(self, segments: list):
        ...
        for segment in segments:
            ...
            self.segment_size.labels(segment_name=segment_name).set(
                segment.get('user_count', 0)
            )
            self.segment_engagement.labels(segment_name=segment_name).set(
                segment.get('avg_engagement', 0)
            )
    ...
    def update_funnel_metrics(self, funnels: Dict):
        ...
        for funnel_name, funnel_data in funnels.items():
            ...
            for step in funnel_data.get('steps', []):
                ...
                self.funnel_step_completion.labels(
                    funnel_name=funnel_name,
                    step=step_name
                ).set(step.get('completion_rate', 0))
                self.funnel_drop_off.labels(
                    funnel_name=funnel_name,
                    step=step_name
                ).set(step.get('drop_off_rate', 0))
