...
class DataAggregator:
    ...
    async def _fetch_plausible_data(self, time_range: str) -> Dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                response = await client.get(
                    f"{self.plausible_url}/api/v1/stats/aggregate", params={"period": time_range}
                )
        ...
        return {
            ...
            "bounce_rate": 42.5,
            "visit_duration": 185,
            "top_pages": {"/": 2100, "/catalog": 1800, "/create": 950, "/docs": 850, "/templates": 720},
    ...
    async def get_usage_trends(self, time_range: str) -> UsageTrends:
        ...
        for i in range(20):
            ...
            time_series.append(
                TimeSeriesDataPoint(timestamp=ts, value=plausible_data["visitors"] * (0.8 + 0.4 * (i / 20)))
            )
    ...
    async def get_feature_adoption(self, time_range: str) -> FeatureAdoption:
        ...
        features = [
            FeatureUsage(
                feature_name="Deploy Application", total_uses=850, unique_users=245, adoption_rate=65.0, trend="up"
            ),
            FeatureUsage(
                feature_name="Create Service", total_uses=620, unique_users=198, adoption_rate=52.0, trend="up"
            ),
            FeatureUsage(
                feature_name="View Documentation", total_uses=1200, unique_users=312, adoption_rate=82.0, trend="stable"
            ),
            FeatureUsage(
                feature_name="Run Pipeline", total_uses=1450, unique_users=267, adoption_rate=70.0, trend="up"
            ),
            FeatureUsage(
                feature_name="Configure Monitoring", total_uses=340, unique_users=125, adoption_rate=33.0, trend="down"
            ),
        ]
        ...
        self.metrics_collector.update_feature_metrics(
            [
                {"feature_name": f.feature_name, "adoption_rate": f.adoption_rate, "unique_users": f.unique_users}
                for f in features
            ]
        )
        ...
        for i in range(15):
            ...
            adoption_trend.append(TimeSeriesDataPoint(timestamp=ts, value=45 + 20 * (i / 15)))  # Growing adoption
    ...
    async def get_experiment_results(self, status: Optional[str] = None) -> List[ExperimentResults]:
        ...
        results = []
        for exp in exp_data:
            variants = [VariantMetrics(**v) for v in exp.get("variants", [])]
            ...
            [
                {
                    ...
                    "status": r.status,
                    "variants": [
                        {"variant_id": v.variant_id, "conversions": v.conversions, "conversion_rate": v.conversion_rate}
                        for v in r.variants
                    ],
                }
                for r in results
    ...
    async def get_user_segments(self, time_range: str) -> UserSegments:
        ...
        self.metrics_collector.update_segment_metrics(
            [
                {"segment_name": s.segment_name, "user_count": s.user_count, "avg_engagement": s.avg_engagement}
                for s in segments
            ]
        )
    ...
    async def export_data(self, format: str, time_range: str) -> Dict:
        ...
        if format == "json":
            ...
            return {"message": "CSV export not yet implemented", "data": data.model_dump()}
