...
class SonarQubeIntegration:
    ...
    def __init__(self, sonarqube_url: str, sonarqube_token: str, http_client: httpx.AsyncClient):
    ...
    async def _fetch_issues(self, project_key: str, pr_number: int) -> List[Dict]:
        ...
        try:
            ...
            response = await self.http_client.get(url, params=params, headers=headers, timeout=15.0)
    ...
    def _prioritize_findings(self, findings: List[Dict]) -> List[Dict]:
        ...
        category_priority = {"security": 0, "quality": 1, "performance": 2, "best_practices": 3, "documentation": 4}

        # Sort by severity first, then category
        sorted_findings = sorted(
            findings, key=lambda f: (severity_priority.get(f["severity"], 99), category_priority.get(f["category"], 99))
        )
    ...
    def deduplicate_with_ai_findings(self, ai_findings: List[Dict], sonarqube_findings: List[Dict]) -> List[Dict]:
        ...
        for sq_finding in sonarqube_findings:
            ...
            for ai_finding in ai_findings:
                ...
                if (
                    sq_finding["file"] == ai_finding.get("path")
                    and abs(sq_finding["line"] - ai_finding.get("line", 0)) <= 2
                ):
