...
class ReviewEngine:
    ...
    async def review_pull_request(self, pr_data: Dict, repo_data: Dict) -> ReviewResult:
        ...
        comments = await self._generate_review_comments(diff, files, rag_context, sonarqube_findings)
    ...
    async def _query_rag_for_context(self, files: List[Dict]) -> str:
        ...
        try:
            ...
            response = await self.http_client.post(
                f"{self.rag_service_url}/api/v1/query", json={"query": query, "top_k": 5}, timeout=10.0
            )
    ...
    async def _fetch_sonarqube_findings(self, repo: str, pr_number: int) -> List[Dict]:
        ...
        try:
            ...
            integration = SonarQubeIntegration(
                sonarqube_url=self.sonarqube_url, sonarqube_token=self.sonarqube_token, http_client=self.http_client
            )
            ...
            for category in categories:
                try:
                    file_comments = await self._analyze_file_with_llm(
                        filename, patch, category, rag_context, prompt_loader
                    )
...
"""
            ...
            response = await self.http_client.post(self.llm_api_url, headers=headers, json=payload, timeout=30.0)
