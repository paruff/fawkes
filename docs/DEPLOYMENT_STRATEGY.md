...
**Update (2026-09-05):** all **17** Python services under `services/` now have CI-run lint/tests (`service-python-tests.yml`, added in #1747) but do not yet build/push images or participate in GitOps promotion — extending the python-fawkes-path/dora-metrics pattern to them is a natural next step, in progress (#1751 Phase 2).
...
- The "Update GitOps" commit-and-push step had no retry, so it silently lost the race whenever another automated commit (a sibling service's own GitOps update, or `release-please`) landed on `main` between checkout and push. Fixed with a fetch-rebase-retry loop (5 attempts) in `python-fawkes-path-ci.yml`, `smart-alerting-ci.yml`, `dora-metrics-ci.yml`.
...
- A related, separately-diagnosed CI flake (Trivy vulnerability scan failing non-reproducibly on identical image digests) turned out to be a real CVE in pip's vendored dependency snapshot inside the runtime image (`CVE-2026-13346`), invisible in the UI because SARIF output produces no console findings by default. Fixed by stripping pip/setuptools from the runtime stage of `python-fawkes-path`'s Dockerfile (never invoked at runtime in a multi-stage build).

The full build→scan→sign→SBOM→GitOps pattern was also factored into a new reusable workflow (`reusable-python-service-golden-path.yml`) and piloted on a 4th service, `anomaly-detection`, in addition to the existing `python-fawkes-path`/`dora-metrics`/`smart-alerting`. 14 of the 17 services still only get lint+test (tracked in #1792).

PR #1798 re-triggers `python-fawkes-path` and `smart-alerting` to confirm all of the above lands correctly end-to-end (build → scan → sign → SBOM → GitOps commit succeeds on the first real attempt → ArgoCD syncs → pod `Running`) — see that PR for the live result.

**Update (2026-09-07, #1804 Phase 1 tracer bullet):** python-fawkes-path now runs the target Phase 2 model for real, not just in-tree. The full chain was exercised live on a local kind cluster: a commit to [`paruff/python-fawkes-path`](https://github.com/paruff/python-fawkes-path) triggers the Tekton `golden-path` pipeline (`platform/apps/tekton/golden-path-pipeline.yaml`) — fetch-source → lint-and-test → SonarCloud scan → build-and-push (real GHCR image, `ghcr.io/paruff/python-fawkes-path`) → Trivy image scan → `gitops-promote`, which opens a real PR against [`paruff/python-fawkes-path-gitops`](https://github.com/paruff/python-fawkes-path-gitops). Merging that PR lets ArgoCD (via `platform/apps/python-fawkes-path/python-fawkes-path-application.yaml`, plus phase-scoped `python-fawkes-path-{alpha,beta,prod}-applicationset.yaml`) auto-sync the new image into the cluster, producing a running, traced, metriced, logged pod. This is the separate-GitOps-repo model the gap table below previously listed as unmet — it is now proven for this one service. Two real gaps found and left open: SonarCloud quality-gate blocking is deliberately disabled for now (`sonar.qualitygate.wait=false` — new-project baseline was inconsistent; revisit in Phase 2), and DevLake's DORA collection for this repo is blocked by unresolved issue #1855.
...
| Canary deployments | 🟡 — controller, `AnalysisTemplate`, and python-fawkes-path's `Rollout` conversion all exist (`platform/apps/argo-rollouts/argo-rollouts-application.yaml`, `platform/apps/python-fawkes-path/python-fawkes-path-analysis-template.yaml`, `python-fawkes-path-gitops#6`); pending human merge + a live canary run to confirm the analysis step actually gates a rollout | ✅ |
