...
- [x] Phase 5: fawkes-on-fawkes DORA dashboard live and linked from README — **done 2026-09-18**, on a redesigned foundation (#2081's original DevLake-based design is obsolete: DevLake was fully decommissioned, replaced by native PromQL + a dedicated service, matching every other DORA metric in this repo). Built `services/dora-metrics` (FastAPI, computes Deployment Frequency and Lead Time for Changes for `paruff/fawkes` itself from real GitHub PR data, no DevLake/Pushgateway/parallel pipeline), two new `dora:fawkes_repo_*` recording rules, two new Grafana panels on the existing DORA dashboard, and a README link (also corrected a stale "DORA metrics partial" claim there). **Live-verified**: the computation logic itself ran standalone against real GitHub data (5.13 releases/day, 0.64h avg lead time - genuine numbers, not fabricated); the containerized service is deployed and `Running`/`Ready` on the LAN k3s cluster, its `/health` endpoint responds correctly in-cluster. **Not yet observed end-to-end**: a completed `/metrics` HTTP response and a populated Grafana panel - blocked on `gh` CLI's inherent one-API-call-per-PR latency compounding with an extremely active day (this session alone), not a correctness issue; and Prometheus's ServiceMonitor-discovery gap (already documented for `space-metrics` earlier this session) recurred here too. Both are real, separate, already-tracked follow-ups, not fabricated as resolved.
