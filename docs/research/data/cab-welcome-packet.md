# Welcome to the Fawkes Customer Advisory Board!

Dear [Member Name],

Welcome to the Fawkes Customer Advisory Board! We're thrilled to have you join this strategic group of platform engineering leaders who will help shape the future of Fawkes.

This welcome packet contains everything you need to get started as a CAB member.

---

## Table of Contents

1. [What to Expect](#what-to-expect)
2. [Your First Steps](#your-first-steps)
3. [Meeting Schedule](#meeting-schedule)
4. [Communication Channels](#communication-channels)
5. [How to Provide Feedback](#how-to-provide-feedback)
6. [Resources and Links](#resources-and-links)
7. [Recognition and Benefits](#recognition-and-benefits)
8. [FAQs](#faqs)
9. [Contact Information](#contact-information)

---

## What to Expect

### Time Commitment

**Total: 4-6 hours per quarter**

- **Quarterly Strategic Meeting**: 2 hours (virtual)
  - Scheduled Q1, Q2, Q3, Q4
  - Calendar invite sent 4-6 weeks in advance
  - Recordings available if you can't attend live

- **Async Participation**: 1-2 hours
  - Monitor and contribute to Mattermost `#cab-advisory-board` channel
  - Respond to polls and surveys (1-2 per quarter)
  - Review RFCs when tagged

- **Early Access Testing**: 2 hours (optional, 2-3 times per year)
  - Test pre-release features
  - Provide structured feedback
  - Recognition in release notes

### Your Role

As a CAB member, you will:

✅ **Provide Strategic Input**: Guide roadmap priorities and platform direction

✅ **Represent User Needs**: Advocate for diverse use cases and perspectives

✅ **Give Early Feedback**: Review and test pre-release features

✅ **Share Experiences**: Contribute adoption stories and best practices

✅ **Build Community**: Network with other platform engineering leaders

### What's Expected

- **Constructive Feedback**: Thoughtful, actionable input
- **Active Engagement**: Participate in meetings and discussions
- **Confidentiality**: Keep pre-release information private
- **Respect**: Professional, collaborative interactions
- **Honesty**: Frank feedback, even when critical

---

## Your First Steps

### Week 1 Checklist

- [x] **You're Reading This!** Welcome packet review
- [ ] **Join Mattermost**: Access `#cab-advisory-board` private channel
- [ ] **GitHub Team**: Confirm you're part of `@fawkes/advisory-board`
- [ ] **Profile**: Add yourself to CAB member directory (optional, see below)
- [ ] **Intro Call**: Schedule 30-minute onboarding call with product team
- [ ] **Review Roadmap**: Read current quarter roadmap document (linked in Mattermost)
- [ ] **Say Hello**: Post intro in `#cab-advisory-board` channel

### Access Setup

**Mattermost Channel**: `#cab-advisory-board`
- Private channel for CAB members and product team
- Your invite is already sent - check your Mattermost notifications
- If you don't see it, post in `#general` or DM the product lead

**GitHub Team**: `@fawkes/advisory-board`
- You'll be mentioned on relevant issues, discussions, and RFCs
- Check your GitHub notifications settings to ensure you receive mentions
- Team membership: https://github.com/orgs/paruff/teams/advisory-board

**CAB Member Directory** (optional):
- We'd love to publicly recognize you in `docs/CUSTOMER_ADVISORY_BOARD_MEMBERS.md`
- You can provide: Name, Company, Role, Bio, LinkedIn/GitHub links, Photo
- Send info to product team or submit a PR
- Opt-in only - no pressure!

### Your Onboarding Call

**What We'll Cover** (30 minutes):
1. Welcome and thank you
2. Review CAB charter and expectations
3. Answer your questions
4. Discuss your specific interests and expertise areas
5. Review upcoming opportunities for input
6. Walk through communication channels

**Schedule Your Call**:
- [Calendly link or email to schedule]
- Or propose times in the `#cab-advisory-board` channel

---

## Meeting Schedule

### Quarterly Strategic Meetings

**Q1 Meeting**: January [Date], [Time] [Timezone]
**Q2 Meeting**: April [Date], [Time] [Timezone]
**Q3 Meeting**: July [Date], [Time] [Timezone]
**Q4 Meeting**: October [Date], [Time] [Timezone]

**Duration**: 2 hours

**Format**: Virtual (Zoom/Google Meet/Teams link in calendar invite)

**What to Expect**:
- Platform updates and progress
- Roadmap review and prioritization
- Feature deep dives
- Open discussion and feedback
- Q&A with product and engineering team

**Preparation**:
- Pre-read materials sent 1 week before (30 min review)
- No homework required, but context helps

**Can't Attend?**
- Let us know in the channel
- Recording and notes shared within 48 hours
- We appreciate >50% attendance over the year

### Ad-Hoc Sessions

**Feature Feedback Sessions** (as needed):
- 30-60 minutes
- Optional attendance
- Deep dive on specific features
- Scheduled 2+ weeks in advance

**Early Access Reviews** (2-3 times per year):
- Async - test on your schedule
- 1-2 week testing window
- Structured feedback form provided

---

## Communication Channels

### Primary: Mattermost

**Channel**: `#cab-advisory-board` (Private)

**Purpose**:
- Strategic discussions and feedback
- Announcements and updates
- Meeting logistics
- Document sharing
- Quick polls and surveys

**Response Time**:
- Product team monitors daily
- No expectation of immediate response from members
- Contribute when you have time and insights

**Guidelines**:
- Keep pre-release info confidential
- Be respectful and constructive
- No urgent support requests (use normal channels)
- @ mention specific people for direct questions

### Secondary: GitHub

**Team**: `@fawkes/advisory-board`

**When You'll Be Mentioned**:
- Major RFCs requiring CAB input
- Strategic discussions and decisions
- Early access opportunities

**How to Respond**:
- Comment directly on the issue/discussion
- Or discuss in Mattermost if you prefer

### Email

**CAB Email List**: cab@fawkes.local (if needed)

**What You'll Receive**:
- Meeting invitations
- Formal announcements
- Quarterly CAB summaries
- Low volume - max 1-2 emails per month

---

## How to Provide Feedback

### 1. During Quarterly Meetings

- **Voice Your Thoughts**: Open floor for discussion
- **Vote on Priorities**: When asked (using polls or show of hands)
- **Ask Questions**: No question is too basic
- **Share Stories**: Successes and challenges welcome

### 2. In Mattermost Channel

- **Async Discussions**: Post thoughts anytime
- **React to Proposals**: 👍 👎 or comment
- **Start Threads**: Keep conversations organized
- **Share Links**: Relevant articles, tools, comparisons

### 3. Surveys and Polls

- **When**: 1-2 per quarter (as needed)
- **Response Time**: Typically 1 week
- **Format**: Simple forms or Mattermost polls
- **Purpose**: Quick input on specific decisions

### 4. Early Access Testing

**Process**:
1. Product team shares early preview (branch, demo environment, or local setup instructions)
2. Test in your context (1-2 weeks)
3. Complete feedback form or add comments in GitHub Discussion
4. Optional: Join sync call to discuss findings

**What to Test**:
- Does it solve the problem?
- Does it work in your environment?
- Any bugs or unexpected behavior?
- User experience and ease of use
- Documentation clarity

### 5. RFC Reviews

**When You're Tagged**:
- You'll get GitHub notification
- Review the RFC (usually 2-10 pages)
- Comment with your perspective
- 1 week typical response window

**What to Focus On**:
- Does the approach make sense?
- Any concerns or risks?
- Alternative approaches to consider?
- Impact on your use case

---

## Resources and Links

### Key Documents

- **CAB Charter**: [Link to docs/CUSTOMER_ADVISORY_BOARD.md]
- **Project Governance**: [Link to docs/GOVERNACE.md]
- **Project Charter**: [Link to docs/CHARTER.md]
- **Current Roadmap**: [Link to roadmap doc or Mattermost pinned post]

### Fawkes Resources

- **Documentation**: https://fawkes.readthedocs.io (or docs site URL)
- **GitHub Repository**: https://github.com/paruff/fawkes
- **Community Discussions**: https://github.com/paruff/fawkes/discussions
- **Mattermost**: [Link to Fawkes Mattermost]

### CAB-Specific Resources

- **Member Directory**: `docs/CUSTOMER_ADVISORY_BOARD_MEMBERS.md`
- **Nomination Template**: `docs/research/templates/cab-nomination.md`
- **Meeting Agenda Template**: `docs/research/templates/cab-meeting-agenda.md`
- **Feedback Form Template**: `docs/research/templates/cab-feedback-form.md`

### Meeting Materials

- **Shared Folder**: [Link to shared folder if applicable]
- **Past Meetings**: Recordings and notes in Mattermost channel (pinned)

---

## Recognition and Benefits

### What You Get

🎯 **Strategic Influence**
- Direct impact on platform direction
- Your feedback shapes the roadmap
- Early input on major decisions

🔬 **Early Access**
- Preview features before general release
- Opportunity to shape implementation
- Listed as early access tester in release notes

🌟 **Public Recognition**
- Listed in CAB member directory
- Credited when your input shapes features
- "CAB Spotlight" blog posts
- Speaking opportunities (optional)

🤝 **Networking**
- Connect with other platform engineering leaders
- Share experiences and best practices
- Build your professional network

🎁 **Swag and Badges** (optional)
- "Fawkes CAB Member" digital badge for LinkedIn/GitHub
- Custom swag package (if offered)
- Exclusive member items

📚 **Learning**
- Behind-the-scenes look at platform engineering
- Access to product strategy discussions
- Insights into roadmap planning

### No Monetary Compensation

The CAB is a volunteer advisory role. There is no payment or monetary compensation. The value is in the influence, access, recognition, and networking.

---

## FAQs

**Q: How much time does this really take?**
A: Most members spend 4-6 hours per quarter: 2 hours for the meeting, 1-2 hours for async participation, and 2 hours for testing (optional). It's flexible - contribute as much as your schedule allows.

**Q: What if I can't attend a quarterly meeting?**
A: No problem! We record meetings (with permission) and share notes. We hope you can attend >50% of meetings over the year, but we understand schedules change.

**Q: Can I share what we discuss publicly?**
A: Most CAB discussions are fine to share. The exception is pre-release features and confidential roadmap details. When in doubt, ask in the channel or wait until it's publicly announced.

**Q: What if I want to leave the CAB?**
A: You're welcome to step down at any time, no questions asked. Just let us know so we can properly thank you and possibly transition your spot to another member.

**Q: Can I nominate someone else to join?**
A: Absolutely! We love member referrals. Have them fill out the nomination template or send us their info.

**Q: How do decisions get made?**
A: The product team makes final decisions, considering all input including CAB feedback. We'll explain our rationale when CAB input influences decisions. Your voice matters, but you're not voting on every decision.

**Q: What if I disagree with a decision?**
A: Healthy debate is encouraged! Share your perspective. Sometimes we'll disagree and choose a different path - that's okay. We'll explain why.

**Q: Can I bring a colleague to meetings?**
A: For quarterly meetings, members only (plus product team). However, we can invite "guest advisors" for specific topics if their expertise is needed. Just ask!

---

## Contact Information

### Product Team

**Product Lead**: [Name]
- Mattermost: @[username]
- Email: [email]
- GitHub: @[username]

**Engineering Lead**: [Name]
- Mattermost: @[username]
- Email: [email]
- GitHub: @[username]

**Community Manager**: [Name]
- Mattermost: @[username]
- Email: [email]
- GitHub: @[username]

### Getting Help

- **Quick Questions**: Post in `#cab-advisory-board` channel
- **Private Matters**: DM the product lead on Mattermost
- **Email**: cab@fawkes.local or product-team@fawkes.local
- **Urgent Issues**: Reach out directly to product lead

---

## What's Next?

**This Week**:
1. Complete the first steps checklist above
2. Join your onboarding call
3. Review the current roadmap document
4. Introduce yourself in the `#cab-advisory-board` channel

**This Month**:
- Get familiar with the Mattermost channel and discussions
- Review recent meeting notes (if any)
- Share initial thoughts or feedback

**This Quarter**:
- Attend (or watch recording of) your first quarterly meeting
- Participate in at least one async discussion
- Consider early access testing opportunities

---

## Thank You!

We're genuinely excited to have you on the Customer Advisory Board. Your experience, insights, and feedback will help make Fawkes better for everyone.

Don't hesitate to reach out with questions, ideas, or feedback at any time. We're here to make this a valuable and rewarding experience for you.

**Welcome aboard!**

— The Fawkes Product Team

---

**Document Version**: 1.0
**Last Updated**: December 25, 2025
**Questions?** Contact product-team@fawkes.local or post in `#cab-advisory-board`
